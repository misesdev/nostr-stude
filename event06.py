...
chave_publica_bytes = chave_publica_obj.serialize(compressed=True)
...
public_key_hex = chave_publica_bytes.hex()[2:]

print(f"pubkey: {public_key_hex}")
...
evento01 = {
	...
	"kind" : 0, 
    "tags" : [],
    "content" : "{ \"name\": \"malboro\", \"displayName\": \"Mal Boro\" }",
	"sig" : ""
}
...
#assinatura
assinatura_obj = chave_privada_obj.ecdsa_sign(id_evento.encode())
...
chave_publica_obj.ecdsa_verify(id_evento.encode(), assinatura_obj)
...
def hello():
	with connect("wss://relay.primal.net") as websocket:
        ...
        #print("user private key:", private_key_hex)
        #print("user public key:", public_key_hex)
